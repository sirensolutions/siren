...
import static com.sindicetech.siren.search.AbstractTestSirenScorer.NodeNumericRangeQueryBuilder.nmqDouble;
import static com.sindicetech.siren.search.AbstractTestSirenScorer.NodeNumericRangeQueryBuilder.nmqLong;
...
public class TestNodeNumericRangeQuery64 extends SirenTestCase {
  ...
  public void testInfiniteValues() throws Exception {
    ...
    Query q = twq(1)
    .with(child(must(nmqLong("field4", 4, null, null, true, true)
      .bound(2, 2)))).getLuceneProxyQuery();
    TopDocs topDocs = index.searcher.search(q, 10);
    assertEquals("Score doc count", 3,  topDocs.scoreDocs.length );

    q = twq(1)
    .with(child(must(nmqLong("field4", 4, null, null, false, false)
      .bound(2, 2)))).getLuceneProxyQuery();
    topDocs = index.searcher.search(q, 10);
    assertEquals("Score doc count", 3,  topDocs.scoreDocs.length );

    q = twq(1)
    .with(child(must(nmqLong("field4", 4, Long.MIN_VALUE, Long.MAX_VALUE, true, true)
      .bound(2, 2)))).getLuceneProxyQuery();
    topDocs = index.searcher.search(q, 10);
    assertEquals("Score doc count", 3,  topDocs.scoreDocs.length );

    q = twq(1)
    .with(child(must(nmqLong("field4", 4, Long.MIN_VALUE, Long.MAX_VALUE, false, false)
      .bound(2, 2)))).getLuceneProxyQuery();
    topDocs = index.searcher.search(q, 10);
    ...
    q = twq(1)
    .with(child(must(nmqDouble("double4", 4, null, null, false, false)
      .bound(2, 2)))).getLuceneProxyQuery();
    topDocs = index.searcher.search(q, 10);
    assertEquals("Score doc count", 3,  topDocs.scoreDocs.length );

    q = twq(1)
    .with(child(must(nmqDouble("double4", 4, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, true, true)
      .bound(2, 2)))).getLuceneProxyQuery();
    topDocs = index.searcher.search(q, 10);
    assertEquals("Score doc count", 3,  topDocs.scoreDocs.length );

    q = twq(1)
    .with(child(must(nmqDouble("double4", 4, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, false, false)
      .bound(2, 2)))).getLuceneProxyQuery();
    topDocs = index.searcher.search(q, 10);
    assertEquals("Score doc count", 1,  topDocs.scoreDocs.length );

    q = twq(1)
    .with(child(must(nmqDouble("double4", 4, Double.NaN, Double.NaN, true, true)
      .bound(2, 2)))).getLuceneProxyQuery();
    topDocs = index.searcher.search(q, 10);
  ...
  private void testRandomTrieAndClassicRangeQuery(final int precisionStep) throws Exception {
    ...
    for (int i = 0; i < num; i++) {
      ...
      MultiNodeTermQuery tq = (MultiNodeTermQuery) nmqLong(field, precisionStep, lower, upper, true, true).getQuery();
...
//      NumericUtils.sortableLongToDouble(lower), NumericUtils.sortableLongToDouble(upper), true, true);
  ...
  public void testEmptyEnums() throws Exception {
    ...
    // test empty enum
    assert lower < upper;
    assertTrue(0 < this.countTerms((MultiNodeTermQuery) nmqLong("field4", 4, lower, upper, true, true).getQuery()));
    assertEquals(0, this.countTerms((MultiNodeTermQuery) nmqLong("field4", 4, upper, lower, true, true).getQuery()));
    // test empty enum outside of bounds
    lower = distance*noDocs+startOffset;
    upper = 2L * lower;
    assert lower < upper;
    assertEquals(0, this.countTerms((MultiNodeTermQuery) nmqLong("field4", 4, lower, upper, true, true).getQuery()));
  }
