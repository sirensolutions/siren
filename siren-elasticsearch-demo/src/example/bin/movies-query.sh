...
for q in datasets/movies/q*.json
do
    print "------------------------------------------------------\n"
    ...
    echo
    QUERY=$(escape $q)
    curl -X POST "http://localhost:9200/movies/_search?pretty" -d  "{\"query\":{\"tree\":$QUERY}}" 2> /dev/null | sed $SED_FLAGS "s/(^.*\"total\".*$)/$LG\1$D/;s/(^.*\"_id\".*$)/$LG\1$D/"
done
