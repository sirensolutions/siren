...
/**
 * <code>ExtendedTreeField</code> is the basic type for configurable tree-based data
 * analysis.
 *
 * <p>
 *
 ...
 */
public class ExtendedTreeField extends TextField implements ResourceLoaderAware {
  ...
  protected void init(final IndexSchema schema, final Map<String,String> args) {
    ...
    if (datatypeAnalyzerConfigPath == null) {
      throw new SolrException(SolrException.ErrorCode.SERVER_ERROR,
                              "ExtendedTreeField types require a '"+DATATYPECONFIG_KEY+"' parameter: " + this.typeName);
    }
  ...
  private void checkFieldTypeProperties() {
    if (this.isMultiValued()) {
      throw new SolrException(SolrException.ErrorCode.SERVER_ERROR,
                              "ExtendedTreeField types can not be multiValued: " +
                              this.typeName);
    }
    if (!this.hasProperty(FieldProperties.OMIT_NORMS)) {
      throw new SolrException(SolrException.ErrorCode.SERVER_ERROR,
                              "ExtendedTreeField types must omit norms: " +
                              this.typeName);
    }
    if (this.hasProperty(FieldProperties.OMIT_TF_POSITIONS)) {
      throw new SolrException(SolrException.ErrorCode.SERVER_ERROR,
                              "ExtendedTreeField types can not omit term frequencies " +
                              "and positions: " + this.typeName);
    }
    if (this.hasProperty(FieldProperties.OMIT_POSITIONS)) {
      throw new SolrException(SolrException.ErrorCode.SERVER_ERROR,
                              "ExtendedTreeField types can not omit positions: " +
                              this.typeName);
    }
    if (this.hasProperty(FieldProperties.STORE_TERMVECTORS)) {
      throw new SolrException(SolrException.ErrorCode.SERVER_ERROR,
                              "ExtendedTreeField types can not store term vectors: " +
                              this.typeName);
    }
  }
  ...
  public IndexableField createField(final SchemaField field, final Object value,
    ...
    if (!field.indexed()) {
      throw new SolrException(SolrException.ErrorCode.SERVER_ERROR,
                              "ExtendedTreeField instances must be indexed: " +
                              field.getName());
    }
    if (field.multiValued()) {
      throw new SolrException(SolrException.ErrorCode.SERVER_ERROR,
                              "ExtendedTreeField instances can not be multivalued: " +
                              field.getName());
    }
    if (!field.omitNorms()) {
      throw new SolrException(SolrException.ErrorCode.SERVER_ERROR,
                              "ExtendedTreeField instances must omit norms: " +
                              field.getName());
    }
    if (field.omitTermFreqAndPositions()) {
      throw new SolrException(SolrException.ErrorCode.SERVER_ERROR,
                              "ExtendedTreeField instances must not omit term " +
    ...
    if (field.storeTermVector()) {
      throw new SolrException(SolrException.ErrorCode.SERVER_ERROR,
                              "ExtendedTreeField instances can not store term vectors: " +
                              field.getName());
    }
  ...
  protected IndexableField createField(final String name, final String val,
    ...
    if (!type.indexed()) {
      throw new SolrException(SolrException.ErrorCode.SERVER_ERROR,
                              "ExtendedTreeField instances must be indexed: " + name);
    }
    if (!type.tokenized()) {
      throw new SolrException(SolrException.ErrorCode.SERVER_ERROR,
                              "ExtendedTreeField instances must be tokenised: " + name);
    }
    if (!type.omitNorms()) {
      throw new SolrException(SolrException.ErrorCode.SERVER_ERROR,
                              "ExtendedTreeField instances must omit norms: " + name);
    }
  ...
  public void setAnalyzer(final Analyzer analyzer) {
    // Append the SIREn's filters
    this.analyzer = this.appendSirenFilters(analyzer, this.getDatatypes());
  }

  @Override
  public void setQueryAnalyzer(final Analyzer analyzer) {
    // ExtendedTreeField does not require a query analyzer
    queryAnalyzer = null;
  }
  ...
  public void inform(final ResourceLoader resourceLoader) {
    ...
    final TokenizerChain chain = (TokenizerChain) this.analyzer;
   ...
   * <br/>
   * Since we add filters dynamically to the field's analyzer, we have to dynamically remove them from the map of
   * property name -> value. Otherwise, this could cause issues with the
   * {@link org.apache.solr.schema.ManagedIndexSchema} that tries to instantiate it using reflection and can not
   * properly configure them, e.g., registering the datatypes in
  ...
  public SimpleOrderedMap<Object> getNamedPropertyValues(boolean showDefaults) {
    SimpleOrderedMap<Object> namedPropertyValues = super.getNamedPropertyValues(showDefaults);
    ((SimpleOrderedMap) namedPropertyValues.get(ANALYZER)).remove(FILTERS);
